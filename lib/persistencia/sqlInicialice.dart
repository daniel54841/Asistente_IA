...
import '../controllers/models/busqueda.dart';
...
class SqlInicialice {
  ...
  static String createTableBusqueda = "CREATE TABLE Busqueda (producto TEXT, posicion INTEGER)";
  ...
  static Future _onCreate(Database db, int version) async {
    ...
    await db.execute(createTableBusqueda);
  }
...
//Operaciones de Compras
  ...
  static Future<List<Busqueda>> getProductosBusqueda() async {
    ...
    try {
      ...
      for (Map<String, dynamic> mapa in queryResults) {
        Busqueda tiendaToAdd = Busqueda.fromMap(mapa);
        list.add(tiendaToAdd);
      }

      GeneralConstants.logger.i("Numero de elementos de la BD en tabla Busqueda: ${list.length}");
    } catch (e) {
      // Handle any exceptions that might occur during the query
      GeneralConstants.logger.e("Excepcion al obtener todas las tiendas: $e");
      return [];
